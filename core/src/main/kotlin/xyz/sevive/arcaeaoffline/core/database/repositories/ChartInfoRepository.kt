...
interface ChartInfoRepository {
    ...
    fun find(playResult: PlayResult): Flow<ChartInfo?> = find(playResult.songId, playResult.ratingClass)

    fun find(difficulty: Difficulty): Flow<ChartInfo?> = find(difficulty.songId, difficulty.ratingClass)
...
class ChartInfoRepositoryImpl(
    private val dao: ChartInfoDao,
) : ChartInfoRepository {
    override fun find(
        songId: String,
        ratingClass: ArcaeaRatingClass,
    ): Flow<ChartInfo?> = dao.find(songId, ratingClass)
