...
class DifficultyLocalizedRepositoryImpl(
    private val dao: DifficultyLocalizedDao,
) : DifficultyLocalizedRepository {
    override fun find(
        songId: String,
        ratingClass: ArcaeaRatingClass,
    ): Flow<DifficultyLocalized?> = dao.find(songId, ratingClass)

    override fun findAll(): Flow<List<DifficultyLocalized>> = dao.findAll()

    override fun findAllBySongId(songId: String): Flow<List<DifficultyLocalized>> = dao.findAllBySongId(songId)
