...
object PlayResultSerializer : KSerializer<PlayResult> {
    override val descriptor: SerialDescriptor =
        buildClassSerialDescriptor("PlayResult") {
            element<Long>("id") // 0
            element<String>("uuid")
    ...
    override fun deserialize(decoder: Decoder): PlayResult =
        decoder.decodeStructure(descriptor) {
            ...
            while (true) {
                when (val index = decodeElementIndex(descriptor)) {
                    0 -> {
                        cls = cls.copy(id = decodeLongElement(descriptor, 0))
                    }
                    ...
                    2 -> {
                        cls = cls.copy(songId = decodeStringElement(descriptor, 2))
                    }
                    ...
                    4 -> {
                        cls = cls.copy(score = decodeIntElement(descriptor, 4))
                    }

                    5 -> {
                        cls = cls.copy(pure = decodeIntElement(descriptor, 5))
                    }

                    6 -> {
                        cls = cls.copy(far = decodeIntElement(descriptor, 6))
                    }

                    7 -> {
                        cls = cls.copy(lost = decodeIntElement(descriptor, 7))
                    }
                    ...
                    9 -> {
                        cls = cls.copy(maxRecall = decodeIntElement(descriptor, 9))
                    }
                    ...
                    12 -> {
                        cls = cls.copy(comment = decodeStringElement(descriptor, 12))
                    }
