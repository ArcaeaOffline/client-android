...
private fun Mat._median(): Scalar {
    ...
    val median =
        if (numbersCount % 2 == 0) {
            (arr[numbersCount / 2 - 1] + arr[numbersCount / 2]) / 2.0
        } else {
            arr[numbersCount / 2]
        }
...
object ImageHashers {
    ...
    fun average(
        imgGray: Mat,
        hashSize: Int,
    ): Mat = average(imgGray, hashSize.toDouble())
    ...
    fun difference(
        imgGray: Mat,
        hashSize: Int,
    ): Mat = difference(imgGray, hashSize.toDouble())
    ...
    ): Mat = dct(imgGray, hashSize.toDouble(), highFreqFactor.toDouble())
