...
class MigrationTest {
    private val TEST_DB = "arcaea-offline"
    ...
    fun migrate7To8() {
        ...
        tables.forEach { table ->
            db.query("SELECT COUNT(*) FROM $table WHERE song_id = 'inkarusi' AND rating_class = 2")
                .use { cursor ->
                    assertTrue(cursor.moveToFirst())
                    assertEquals(1, cursor.getInt(0))
                }

            db.query("SELECT COUNT(*) FROM $table WHERE song_id = 'inkarusi' AND rating_class = 'FUTURE'")
                .use { cursor ->
                    assertTrue(cursor.moveToFirst())
    ...
    private fun execSqlScript(db: SupportSQLiteDatabase, filename: String) {
