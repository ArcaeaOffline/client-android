...
object OcrQueueTaskStatusConverters {
    @TypeConverter
    fun fromDatabaseValue(value: Int?): OcrQueueTaskStatus? = value?.let { OcrQueueTaskStatus.entries[it] }

    @TypeConverter
    fun toDatabaseValue(value: OcrQueueTaskStatus?): Int? = value?.value
}
